...
| [`cec_fair_nf_emap.sh`](../scripts/cec_fair_nf_emap.sh) | Bash | 對已有 fair 輸出的 nf/emap `.v` vs 共用 `synth.aig` 做 ABC CEC | 使用者 |
| [`merge_emap_twins.py`](../scripts/merge_emap_twins.py) | Python | 合併 emap twin FA/HA Verilog，供 `read -m` / stime / CEC | `cec_fair_nf_emap.sh` / compare |
...
- `emap|emap_l1/<case>/*_emap.v` vs 同一份 `synth.aig`（先經 [`merge_emap_twins.py`](../scripts/merge_emap_twins.py) 合併 FA/HA twin）

```bash
./scripts/cec_fair_nf_emap.sh
./scripts/cec_fair_nf_emap.sh --root output/fair_nf_emap_asap7genlib --parallel
./scripts/cec_fair_nf_emap.sh --emap-subdir emap_l1 --cases "adder square" --jobs 4
```
